...
void render_player() {
	...
	if (tick % 500 > 460) {
		blink.setRotation(playerS.getRotation());
		blink.setPosition(playerS.getPosition());
		
		if (tick % 500 < 480) {
			blink.setTextureRect(IntRect(75 * floor((tick % 500 - 480) / 2), 0, 75, 75));
		}
		if (tick % 500 >= 480) {
			blink.setTextureRect(IntRect(75 * (8 - floor((tick % 500 - 480) / 2)), 0, 75, 75));
		}
		window.draw(blink);
	}
	//blinking
