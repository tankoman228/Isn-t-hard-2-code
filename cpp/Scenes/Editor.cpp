...
void EditorTick(float dt) {
    ...
    if (E_play.if_click()) {
         /*lvl_name = helper_string;*/ lvlnum = -2; save(current_filename); InitGameSceneLevel(-2); 
        ChangeScene("game");
        return;
    }
    ...
    if (E_load.if_click()) {
        /*saveornot = false;
        context_menu = true;*/
        // TODO: не автосейы, а спросить или типа того; save(current_filename);
        ChangeScene("editor_menu");
    }
